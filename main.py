import asyncio
asyncio.set_event_loop_policy(asyncio.DefaultEventLoopPolicy())  # ← это исправляет ошибку extra_headers
...
SSID = r"""42["auth",{"session":"a:4:{s:10:\"session_id\";s:32:\"bacc9e016e79b33fd991e0552ec5f43c\";s:10:\"ip_address\";s:14:\"80.253.249.174\";s:10:\"user_agent\";s:120:\"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36 OPR/126.\";s:13:\"last_activity\";i:1770556174;}0d555b7dbbb09f7091d04df5a4a77384","isDemo":0,"uid":119544254,"platform":2,"isFastHistory":true,"isOptimized":true}]""" # Твой новый SSID

po = PocketOption(ssid=SSID, is_demo=False)  # False — реальный, True — демо

# Группы активов (OTC-версии)
ASSET_GROUPS = {
    "Валюты OTC": [
...
# ────────────────────────────────────────────────
# ФУНКЦИЯ ПРИНЯТИЯ РЕШЕНИЯ (5+1 индикатор + MTF)
# ────────────────────────────────────────────────
...
async def should_buy(candles_1m, candles_5m=None):
    ...
    # Считаем MACD-линию по всей доступной истории
    macd_lines = []
    ...
    if len(macd_lines) < 9:
        ...
        macd_signal = ema(macd_lines[-9:], 9)  # Правильная EMA9 по последним 9 значениям MACD-линии
    ...
    if data.startswith("asset:"):
        ...
        if result:
            ...
            text = (
                ...
                f"Согласование: {count}/6 индикаторов\n"
            ...
            await query.edit_message_text(f"Сигнала по {asset} сейчас нет (менее 5 индикаторов согласны)")
        return
...
async def on_startup(app: Application):
    try:
        print("Подключение к PocketOption...")
        await po.connect()
        print("Подключение к PocketOption завершено")
    except Exception as e:
